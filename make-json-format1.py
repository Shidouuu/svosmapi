...
def test_format(format):
    try:
        ...
        print(f"Caught KeyError in test_format for {format}.", file=log)
        return False
            ...
            try:
                ...
                with os.scandir(f"svosmapi/assets/{FolderType}/{Path(SubName).name}") as AssetDir:
                    for AssetName in AssetDir:
                        ...
                        if os.path.isdir(AssetName):
                            ...
                            with os.scandir(f"svosmapi/assets/{new_path}") as AssetSubDir:
                                for SubAsset in AssetSubDir:
                                    ...
                                    if os.path.isdir(SubAsset):
                                        ...
                                        with os.scandir(f"svosmapi/assets/{new_path}") as AssetSub3Dir:
                                            for Sub3Asset in AssetSub3Dir:
                                                ...
                                                if type(fifth) == dict and test_format(fifth):
                                                    try:
                                                        type(fourth[fifth["Format"]]) != dict
                                                    except KeyError:
                                                        print(f"Caught KeyError in scan_standard for {fifth}.", file=log)
                                                        fourth[fifth["Format"]] = {}
                                                    fourth[fifth["Format"]][Path(Sub3Asset).stem] = fifth
                                                else:
                                    ...
                                    if type(fourth) == dict and test_format(fourth):
                                        try:
                                            type(third[fourth["Format"]]) != dict
                                        except KeyError:
                                            print(f"Caught KeyError in scan_standard for {fourth}.", file=log)
                                            third[fourth["Format"]] = {}
                                        third[fourth["Format"]][Path(SubAsset).stem] = fourth
                                    else:
                        ...
                        if type(third) == dict and test_format(third):
                            try: 
                                type(second_dict[third["Format"]]) != dict
                            except KeyError:
                                print(f"Caught KeyError in scan_standard for {third}.", file=log)
                                second_dict[third["Format"]] = {}
                            second_dict[third["Format"]][Path(AssetName).stem] = third
                        else:
